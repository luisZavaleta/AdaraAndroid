...
public class CameraPreviewHandler implements PreviewCallback {
	...
	protected void init(Camera camera)  {
		...
		if(Config.USE_ONE_SHOT_PREVIEW) {
			constFPS  = new CameraConstFPS(Config.frameRate, camera);//Frames per second
			constFPS.start();
		}	
	...
	class CameraConstFPS extends Thread {
		...
		public synchronized void run() {
			...
			while(threadsRunning) {
				try {
					if(Config.DEBUG)
					Log.w("AspectJ", "RUN CAMERA PREVIEW    -- -- -- -- -- ->");
					wait(waitTime);
				} catch (InterruptedException e) {}
	...
	class AutoFocusHandler extends Thread implements AutoFocusCallback, MarkerVisibilityListener {
		...
		//private static final int MIN_TIME = 1500;//Changed to the config 
		//private static final int ENSURE_TIME = 10000;//Changed to the config 
		...
		public synchronized void run() {
			...
			while(threadsRunning) {
				...
				if(!visible && (currTime-lastScan)>Config.ensureTime) {
					if(camStatus.previewing)  {
						camera.autoFocus(this);
						lastScan = currTime;
					}
		...
		public void makerVisibilityChanged(boolean visible) {
			...
			if(!visible) {
				long currTime = System.currentTimeMillis();
				if((currTime-lastScan)>Config.minTime) {
					if(camStatus.previewing) {
						camera.autoFocus(this);
						lastScan = currTime;
					}
