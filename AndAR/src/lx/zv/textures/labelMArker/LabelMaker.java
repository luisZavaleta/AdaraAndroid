...
package lx.zv.textures.labelMArker;
 ...
 * changes.
 * 
 * Modified for @luisZavaleta for handling rotation when the screen position is fixed, and a texture pool :)
 *
 */
public class LabelMaker {
    ...
    public int add(GL10 gl, String text, Paint textPaint, Side side) {
        return add(gl, null, text, textPaint, side);
    }

    
    public int add(GL10 gl, String text, Paint textPaint, Side side, TexTexturePool ttp, boolean tpActive) {
    	/*
    	 * addLite(GL10 gl, Drawable background, String text, Paint textPaint,
            int minWidth, int minHeight, Side side, TexTexturePool ttp)
    	 * */
        		return add(gl, null, text, textPaint, side, ttp);
    ...
    public int add(GL10 gl, Drawable background, String text, Paint textPaint) {
        return add(gl, background, text, textPaint, 0, 0, null, null);
    }

    
    public int add(GL10 gl, Drawable background, String text, Paint textPaint, Side side) {
        return add(gl, background, text, textPaint, 0, 0, side, null);
    }
    
    
    public int add(GL10 gl, Drawable background, String text, Paint textPaint, Side side, TexTexturePool ttp) {
        return add(gl, background, text, textPaint, 0, 0, side, ttp);
    }
    ...
    public int add(GL10 gl, Drawable drawable, int minWidth, int minHeight) {
        return add(gl, drawable, null, null, minWidth, minHeight, null, null);
    }
    ...
    public int add(GL10 gl, Drawable background, String text, Paint textPaint,
    	...
    	boolean manualRotation = side != null;
    	
    	boolean texturePool = ttp != null;
        ...
        if(manualRotation && (side.equals(Side.TOP) || side.equals(Side.BOTTOM))){ 
        	
        	textHeight = Math.min(mStrikeWidth,measuredTextWidth);
        ...
        int width = 0;
        	...
        	width = Math.max(minWidth, textWidth + padWidth);
        	height = Math.max(minHeight, textHeight + padHeight);
        ...
        if (u + measuredTextWidth > mStrikeWidth) {
        ...
        if (drawText) {
        	...
        	int x = u + padding.left +centerOffsetWidth; //x
        	
        	
        	int y = vBase + padding.top + centerOffsetHeight; //y
        	
        	float xa = (measuredTextWidth)/2f;//mea
        	float ya = ascent/2f;//ascent
        	...
        	if(manualRotation){
        		...
        		case TOP:
        			...
        			mCanvas.rotate(270, xa , ya);  
        			mCanvas.translate(-xa+ya,-xa + ya);       		
        			break;
        		case BOTTOM:
        			mCanvas.rotate(90, xa , ya);  
        			mCanvas.translate(xa-ya,xa - ya);      
        			break;
        		case RIGHT:
        			 mCanvas.rotate(-180, xa , ya);  
        			break;
        ...
        tvo.side = side;
        if(texturePool){        	
        	ttp.setTexture(tvo, mLabels.size() - 1);
        }
    ...
    public void endAdding(GL10 gl, String text, Side side) {
        checkState(STATE_ADDING, STATE_INITIALIZED);
             ...
             textHm.put(tvo, mTextureID);
            // bitMap.put(text, mBitmap);
     ...
     * @param viewHeight
     */
    public void beginDrawing(GL10 gl, float viewWidth, float viewHeight, String text, Side side) {
        checkState(STATE_INITIALIZED, STATE_DRAWING);
        
        TextureVO tvo = new TextureVO();
        tvo.text = text;
        tvo.side = side;
        gl.glBindTexture(GL10.GL_TEXTURE_2D, textHm.get(tvo));
    ...
    public void draw(GL10 gl, float x, float y, int labelID) {
        ...
        //movemos todas las etiquetas para que se posicionen en x, y independiente de la rotación.
        if(label.side != null){
        	if(label.side.equals(Side.BOTTOM)){
        		 y = y - (label.height);		
        	}else if(label.side.equals(Side.RIGHT)){
        		 x = x + (-label.width);
        	}
        }
    ...
    private static class Label {
        ...
        /**/
        public Label(Grid grid, float width, float height, float baseLine, int cropU, int cropV, int cropW, int cropH, Side side) {
            this.grid = grid;
            this.width = width;
            this.height = height;
            this.baseline = baseLine;
    ...
    private HashMap<TextureVO, Integer> textHm = new HashMap<TextureVO,Integer>();
    
 //   private static HashMap<String,Canvas> canvasAR= new HashMap<String,Canvas>();
